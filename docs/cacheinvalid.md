...
> **重要保证**: 当缓存中所有条目都是受保护的（热点或预加载）时，淘汰操作**不会强制删除任何受保护条目**，而是返回失败。此时调用 `Put` 方法会返回 `ErrCapacityExhausted` 错误。这确保了热点数据和预加载数据不会因为容量不足而被意外驱逐。

### 9.3 全部受保护时的行为

当缓存容量已满且所有条目都是受保护条目时：
- 调用 `Put` 或 `PutWithTTL` 会返回 `ErrCapacityExhausted` 错误
- 不会删除任何热点或预加载数据
- 需要手动清除一些条目或取消部分条目的保护状态后才能继续写入
