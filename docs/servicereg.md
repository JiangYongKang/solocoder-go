...
| Instances | []*ServiceInstance | 变更时的实例列表快照（register/deregister 为变更后状态，expire 为删除前状态） |
...
5. `expiryLoop` 协程退出时先设置 `expiryRunning = false` 再调用 `wg.Done()`，确保 `Stop()` 返回后 `expiryRunning` 状态已更新，避免紧接的 `Start()` 误判为过期协程仍在运行

### 通知一致性保证

多服务过期通知采用"先收集快照，后统一发送"的策略：

1. **过期扫描**：在持有锁的情况下扫描所有服务，标记有过期实例的服务
2. **快照收集**：在删除实例前，为每个发生变更的服务构建过期前实例列表快照和订阅者回调列表
3. **实例删除**：快照收集完成后，在持有锁的情况下删除所有已过期的实例
4. **批量通知**：释放锁后，依次发送所有通知

这种设计确保：
- expire 事件的 Instances 字段包含过期前的完整实例列表（包括即将被删除的过期实例），订阅者可据此获知哪些实例被逐出
- 同一轮过期检测产生的所有通知都基于扫描时刻的一致快照
- 通知发送期间新注册的实例不会出现在前一轮的过期通知中
- 避免了原设计中先删除后构建快照导致全部实例过期时事件 Instances 为空的问题
