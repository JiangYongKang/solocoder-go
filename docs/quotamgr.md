...
为避免死锁，锁获取顺序遵循以下规则。**注意**：以下链式表示步骤发生的先后顺序，每一步结束后锁即释放，并非三把锁同时嵌套持有：

```
① 获取全局写锁 → ① 释放全局写锁 → ② 获取租户级锁 → ③ 获取全局读锁 → ③ 释放全局读锁 → ② 释放租户级锁
```

以 `AcquireResource` 为例的实际持有窗口：
- 阶段①（`getOrCreateTenantUsage` 内部）：仅持有全局写锁，函数返回前释放
- 阶段②③之间：同时持有「租户级锁」和「全局读锁」（共享锁嵌套，安全）
- 阶段③之后：仅持有租户级锁，完成使用量更新后释放

具体规则：
- **允许**：先获取全局锁（在辅助函数内获取并释放），再由调用方获取租户级锁
  - 示例：`ReleaseResource` 先调用 `getTenantUsageIfExists()`（内部持有全局读锁→释放），随后调用方获取租户级写锁
  - 示例：`AcquireResource` 先调用 `getOrCreateTenantUsage()`（内部可能持有全局写锁→释放），随后调用方获取租户级写锁
  - 说明：辅助函数仅负责全局锁操作，租户级锁由调用方在辅助函数返回后单独获取
- **允许**：先获取租户级锁，再获取全局读锁
  - 示例：`AcquireResource` 在获取租户级写锁后，再获取全局读锁读取最新配额配置
  - 说明：全局读锁具备共享特性，此顺序是安全的，不会导致死锁
- **禁止**：在持有租户级锁的情况下获取全局写锁
  - 风险：若 goroutine A 持有租户级锁并等待全局写锁，goroutine B 持有全局写锁并等待租户级锁，将形成死锁
...
- **允许共享锁嵌套**：全局读锁具备共享特性，允许在持有租户级锁后获取全局读锁（如 `AcquireResource` 中的做法），既消除 TOCTOU 竞态又不会导致死锁
- **禁止写锁嵌套**：严格禁止在持有租户级锁的情况下获取全局写锁，避免潜在的死锁风险
