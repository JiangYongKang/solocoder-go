...
**设计说明**：
- 当 `held <= 0` 时直接返回，避免无意义的调度调用
- 释放许可与等待者调度逻辑解耦：只有真正释放了许可，才触发调度
- 防止过度释放：多次调用 Release 不会使 held 变为负数
...
- **完整链路保证**：从第一个等待者到最后一个等待者，严格按照入队顺序依次获取许可
- **优点**：保证公平性，不会出现饥饿
- **缺点**：吞吐量相对较低
- **适用场景**：对公平性要求高的场景

公平模式下的完整获取链路示例（5 个 goroutine，1 个许可）：
...
G2 入队 → [G0, G1, G2]
G3 入队 → [G0, G1, G2, G3]
G4 入队 → [G0, G1, G2, G3, G4]
    ↓
增加 1 个许可，G0 获取，出队 → [G1, G2, G3, G4]
    ↓
G0 释放，G1 获取，出队 → [G2, G3, G4]
    ↓
G1 释放，G2 获取，出队 → [G3, G4]
    ↓
G2 释放，G3 获取，出队 → [G4]
    ↓
G3 释放，G4 获取，出队 → []
...
- 调度时总是从队首取出等待者
- 完整的 FIFO 获取链路验证：所有等待者按入队顺序依次获取许可

### 10.4 释放与调度解耦

- Release 方法只在真正释放许可时（held > 0）才触发调度
- 当 held <= 0 时直接返回，避免无意义的调度调用
- 释放许可与等待者调度逻辑解耦，职责更清晰
- 防止过度释放：多次调用 Release 不会使 held 变为负数
