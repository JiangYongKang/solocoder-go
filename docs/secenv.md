...
- **AES-GCM 加密**：使用 AES-256-GCM 算法对任意字节数据进行加密，提供保密性和完整性保护
...
| 错误变量 | 说明 |
|----------|------|
| `ErrInvalidFormat` | 信封格式无效，长度不足 |
| `ErrInvalidVersion` | 信封格式版本不支持 |
| `ErrKeyNotFound` | 指定的密钥版本不存在 |
| `ErrInvalidSignature` | HMAC 签名验证失败 |
| `ErrInvalidTag` | GCM 认证标签验证失败 |
| `ErrReplayDetected` | 检测到重放攻击 |
| `ErrInvalidKeySize` | 密钥长度不正确（必须为 32 字节） |
| `ErrEmptyData` | 输入数据为空 |
