...
- **重入上限**：可通过 `LockManagerConfig.MaxReentrancy` 配置重入次数的上限，防止无限重入。默认上限为 32。
...
- **最小过期时间**：加锁时记录每个节点上的锁过期时间，总持有时间取各节点中的最小过期时间，并扣除操作耗时和时钟漂移补偿。
...
| `Lock(key, token, ttl)` | 加锁，失败返回错误 |
| `TryLock(key, token, ttl)` | 非阻塞尝试加锁 |
| `Unlock(key, token)` | 释放锁，验证 token |
...
| 字段 | 类型 | 说明 | 默认值 |
|------|------|------|--------|
| `MaxReentrancy` | `int` | 最大重入次数，必须 >= 1 | 32 |
